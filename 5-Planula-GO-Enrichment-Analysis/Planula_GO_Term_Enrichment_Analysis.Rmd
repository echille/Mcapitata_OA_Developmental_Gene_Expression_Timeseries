...
KFS.KO <- read_tsv("5-Planula-GO-Enrichment-Analysis/a-Kegg-ontology/Mcap_KO_annot.tsv", col_names = TRUE)[-1,c(1,2,3)]
...
head(KO.terms)
```
Obtain KO terms for each cluster of DEGs
```{r}
...
write.table(koC1, "Output/koC1.KEGG_mapper_input.txt", quote=FALSE,col.names=FALSE,row.names=FALSE,sep="\t") #TO BE USED FOR KEGG Mapper https://www.genome.jp/kegg/mapper.html.

write.table(koC2, "Output/koC2.KEGG_mapper_input.txt", quote=FALSE,col.names=FALSE,row.names=FALSE,sep="\t") #TO BE USED FOR KEGG Mapper https://www.genome.jp/kegg/mapper.html.
```

Perform Kegg enrichment with clusterprofiler package
```{r}
#Cluster1
KO.05.C1 <- enrichKEGG(koC1$ko, organism = "ko", keyType = "kegg", pvalueCutoff = 0.05) #run enrichment with Kegg sig < 0.05
KO.05.C1 <- KO.05.C1[order(KO.05.C1$p.adjust, -KO.05.C1$Count),] #order by p-value, then count
print(KO.05.C1) #analysis of DE genes
dim(KO.05.C1)
write.csv(KO.05.C1, file = "5-Planula-GO-Enrichment-Analysis/Output/KO.05.C1.csv")

#Cluster2
KO.05.C2 <- enrichKEGG(koC2$ko, organism = "ko", keyType = "kegg", pvalueCutoff = 0.05) #run enrichment with Kegg sig < 0.05
KO.05.C2 <- KO.05.C2[order(KO.05.C2$p.adjust, -KO.05.C2$Count),] #order by p-value, then count
head(KO.05.C2) #analysis of DE genes
dim(KO.05.C2)
#Nothing!
```
