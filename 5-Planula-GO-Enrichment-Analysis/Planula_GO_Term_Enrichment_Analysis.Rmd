...
Plot enriched biological process terms as a dot plot.
```{r}
C1_fig <- filter(C1.GO.05, ontology=="BP") #separate out BP terms in cluster 1
C1_fig$dir.cluster <- "Up (Cluster 1)" #label them with "experiment"

C2_fig <- filter(C2.GO.05, ontology=="BP")  #separate out BP terms in cluster 2
C2_fig$dir.cluster <- "Down (Cluster 2)" #label them with "experiment"
...
GOdot.plot <- GOdot %>% ggplot(aes(x = reorder(term, percentDEInCat), y = percentDEInCat, 
  color = `dir.cluster`, size = over_represented_pvalue)) + 
  coord_flip() +
  geom_point() +
  scale_color_manual(values=c("Up (Cluster 1)" = "red", "Down (Cluster 2)" = "blue")) +
  theme_bw() + 
  ylab("percentDEInCat") + 
  xlab("term") +
  ...
  labs(size = "-log10(p-value)", color = "Direction (Cluster)"); GOdot.plot # re-label size labels to reflect p-value tranformation
