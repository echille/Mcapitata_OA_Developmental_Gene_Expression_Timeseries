...
GOdot$p.adjust <- p.adjust(p = GOdot$over_represented_pvalue, method = "BH") # calculate padj
GOdot$percentDEInCat <- (GOdot$numDEInCat/GOdot$numInCat)*100 #calculate %DEInCat
GOdot$p.adjust <- -log10(GOdot$p.adjust)

GOdot <- rename(GOdot, direction=experiment)
GOdot <- arrange(GOdot, percentDEInCat)
my.order <- GOdot$term
GOdot$term <- factor(GOdot$term, levels = my.order)
# plot: dot plot
GOdot.plot <- GOdot %>% arrange(percentDEInCat) %>% ggplot(aes(x = reorder(term, percentDEInCat), y = percentDEInCat, 
                        color = `direction`, size = p.adjust)) + 
  coord_flip() +
  geom_point() +
  scale_color_manual(values=c(up = "red", down = "blue")) +
  theme_bw() + 
  ylab("percentDEInCat") + 
  xlab("term");GOdot.plot
