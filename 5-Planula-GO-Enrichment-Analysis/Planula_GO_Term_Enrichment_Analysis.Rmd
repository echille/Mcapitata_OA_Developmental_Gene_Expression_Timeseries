...
Read in files only if previous steps not run.
```{r}
...
BP_GO <- all_GO %>%
  filter(ontology=="BP")
BPGO_collection <- GOCollection(BP_GO$category)
fl <- "http://current.geneontology.org/ontology/subsets/goslim_generic.obo"
slim <- getOBOCollection(fl)
slims_bp <- data.frame(goSlim(BPGO_collection, slim, "BP"))
slims_bp$category <- row.names(slims_bp)

## MF
MF_GO <- all_GO %>%
  filter(ontology=="MF")
MFGO_collection <- GOCollection(MF_GO$category)
slims_mf <- data.frame(goSlim(MFGO_collection, slim, "MF"))
...
#Run functions and filter out extraneous rows
BPslim <- mappedIds(slims_bp, BPGO_collection, GOBPOFFSPRING)
BPslim <- filter(BPslim, Count>0 & Term!="biological_process")
MFslim <- mappedIds(slims_mf, MFGO_collection, GOMFOFFSPRING)
MFslim <- filter(MFslim, Count>0 & Term!="molecular_function")
```
...
BPslimX <- merge(BPslimX, BPslim[,c(1,3:4)], by="Term")
BPslimX <- unique(setDT(BPslimX)[order(go_term, -Count)], by = "go_term")
BPslim <- data.frame(slim_term=BPslimX$Term, slim_cat=BPslimX$category, category=BPslimX$go_term)
head(BPslim)
...
MFslimX <- merge(MFslimX, MFslim[,c(1,3:4)], by="Term")
MFslimX <- unique(setDT(MFslimX)[order(go_term, -Count)], by = "go_term")
MFslim <- data.frame(slim_term=MFslimX$Term, slim_cat=MFslimX$category, category=MFslimX$go_term)
head(MFslim)
```
Save slim info with GO enrichment info for heatmap dataframe.
```{r}
GO.BP <- right_join(BPslim, filter(all_GO, ontology=="BP"), by="category")
GO.MF <- right_join(MFslim, filter(all_GO, ontology=="MF"), by="category")
...
## Make heatmap
```{r}
BPplot <- DF_GOhm %>% filter(ontology=="BP" & numInCat>5) %>% ggplot(aes(x = dir, y = term)) + 
...
MFplot <- DF_GOhm %>% filter(ontology=="MF" & numInCat>5) %>% ggplot(aes(x = dir, y = term)) + 
  geom_tile(aes(fill=over_represented_pvalue, width = 1)) + 
...
fig4 <- grid.arrange(BPplot, MFplot, ncol = 2, nrow = 1, widths = c(4.25,7), clip = "off")
ggsave("5-Planula-GO-Enrichment-Analysis/Output/fig4_GOslim.pdf", fig4, width = 20, height = 20, units = c("in"))
```
