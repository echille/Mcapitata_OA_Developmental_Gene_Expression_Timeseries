...
gcount <- as.data.frame(read_delim(file = "7-Symbiont-RNAseq-Analysis/a-QC-Align-Assemble/Trinity/RSEM.isoform.counts.matrix.tsv", delim = "\t"))
rownames(gcount) <- gcount$X1
gcount$X1 <- NULL
head(gcount)
nrow(gcount)
gcount <- round(gcount) # round the gene counts matrix
t2g <- read.csv("7-Symbiont-RNAseq-Analysis/b-Differential-Gene-Expression-Analysis/pln_matrix_ids.csv", header = TRUE, sep = ",")
colnames(gcount) <- t2g$sample_id
...
```{r}
filt <- filterfun(pOverA(0.11,5)) #filter out genes that have a count of 5 or fewer in less than 1/9 samples
...
pNOT1 <- filter(as.data.frame(DEG_pln.results.l_vs_amb), padj!=1)
pNOT2 <- filter(as.data.frame(DEG_pln.results.xl_vs_amb), padj!=1)
pNOT3 <- filter(as.data.frame(DEG_pln.results.xl_vs_l), padj!=1)

rn.pNOT1 <- rownames(pNOT1)
gcount[rn.pNOT1,]
rn.pNOT2 <- rownames(pNOT2)
gcount[rn.pNOT2,]
rn.pNOT3 <- rownames(pNOT3)
gcount[rn.pNOT3,]
```
