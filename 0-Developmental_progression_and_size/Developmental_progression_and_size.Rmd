...
Import the data
```{r}
raw_data <- read.csv("0-Developmental_progression_and_size/Input/cell_cleavage.csv" , header = TRUE, sep = ",")
```
...
Testing for sample size effect
```{r}
sample_size_anova_treatment <- aov(total_counts_per_sample ~ treatment, data = total_counts_per_sample)
summary(sample_size_anova_treatment)
sample_size_anova_onecell <- aov(total_counts_per_sample ~ One_cell, data = total_counts_per_sample)
summary(sample_size_anova_onecell)
sample_size_anova_twocell <- aov(total_counts_per_sample ~ Two_cell, data = total_counts_per_sample)
summary(sample_size_anova_twocell)
sample_size_anova_fourcell <- aov(total_counts_per_sample ~ Four_cell, data = total_counts_per_sample)
summary(sample_size_anova_fourcell)
sample_size_anova_greaterthanfourcell <- aov(total_counts_per_sample ~ Greater_than_4_cell, data = total_counts_per_sample)
summary(sample_size_anova_greaterthanfourcell)
```
...
Beta regression models are suggested for proportional data instead of t-tests and anovas because the data is often not normally distributed and homoscedastic. Here, we use a beta regression test (as described by S. S. Mangiafico in the RCompanion Handbook chapter, [Beta Regression for Percent and Proportion Data](https://rcompanion.org/handbook/J_02.html).
```{r}
model = betareg(gathered_cleavage_counts$pr ~ cleavage_stage + treatment + cleavage_stage:treatment,
...
gathered_cleavage_counts <- rename(gathered_cleavage_counts, c("proportion" = "percent"))
...
```{r}
mpc <- summarySE(gathered_cleavage_counts, measurevar = "percent", groupvars = c("treatment", "cleavage_stage"))
```

Plot mean_percent_cleavage
```{r}
mpc$title <- "a) Cleavage"
mpc$treatment <- gsub("AMB", "pH 7.8", mpc$treatment) #replace AMB, l, and xl with pH 7.8, pH 7.6, and pH 7.3 for plotting
mpc$treatment <- gsub("xl", "pH 7.3", mpc$treatment) #replace AMB, l, and xl with pH 7.8, pH 7.6, and pH 7.3 for plotting
mpc$treatment <- gsub("l", "pH 7.6", mpc$treatment) #replace AMB, l, and xl with pH 7.8, pH 7.6, and pH 7.3 for plotting
mpc$treatment <- factor(mpc$treatment, levels=c("pH 7.8", "pH 7.6", "pH 7.3"))

final_cell_cleavage <- ggplot(mpc, aes(fill=cleavage_stage, y = percent, x=treatment)) + 
  geom_bar(position ="stack", stat = "identity") +
  xlab("Treatment") + ylab("Proportion of Embryos at Each Cleavage Stage") + 
  scale_fill_brewer(name = "", labels = c("One Cell", "Two Cell", "Four Cell", "> Four Cell"), palette = "Paired") + #colour modification
  theme_bw() + #Set background color 
  theme(panel.border = element_blank(), # Set border
                     panel.grid.major = element_blank(), #Set major gridlines
                     panel.grid.minor = element_blank(), #Set minor gridlines
                     axis.line = element_line(colour = "black"), #Set axes color
        plot.background=element_blank(), plot.title = element_text(face = 'bold', size = 12, hjust = 0),
  ...
  legend.key.width = unit(0.5,"cm")) + #Set the plot background
  facet_grid(. ~ title) + #ggtitle("(a) Cleavage") + #add a main title
  theme(plot.title = element_text(face = 'bold', 
                                  size = 14, 
                                  hjust = 0),
        legend.position = c(0.5, 0.98), #Place legend top middle of figure
        legend.direction = "horizontal", #Make legend list items in a row versus columns
        legend.background = element_rect(fill = NA), #Legend block no-fill
        axis.title =  element_text(size = 14),
  ...
  strip.text = element_text(size = 14, face = 'bold')); final_cell_cleavage #set facet size
```
...
Import the data
```{r, message = FALSE}
raw_data <- read_csv("0-Developmental_progression_and_size/Input/size_metric_timeseries_data.csv")
...
egg_volume_boxplot <- ggplot(egg_volume, aes(y = volume, x=treatment)) +
  ylab(expression("Volume " (mm^3))) + xlab("Treatment") + # Set x and y axis labels
  ylim(0,0.13)+
...
fert_volume_t_boxplot <- ggplot(fert_volume, aes(x = treatment, y = volume, color = treatment)) + 
  ylab(expression("Volume " (mm^3))) + xlab("Treatment") + #Label axes
  ylim(0,0.13)+
...
gas_volume_t_boxplot <- ggplot(gas_volume, aes(x = treatment, y = volume, color = treatment)) +
  ylab("Volume mm3")+ xlab("Treatment") + #Label axes
  geom_jitter(aes(alpha = 0.3)) +
  geom_boxplot(alpha = 0) +
  theme_bw() + #Set background color
  theme(panel.border = element_blank(), # Set border
...
```{r}
volume_dat <- bind_rows(egg_volume, fert_volume, gas_volume, planulae_volume)
volume_dat$treatment <- replace_na(volume_dat$treatment, "AMB")
volume_dat$time_point <- gsub("Eggs", "b) Unfertilized Egg", volume_dat$time_point)
volume_dat$time_point <- gsub("Fertilized_embryos", "c) Fertilized Egg", volume_dat$time_point)
volume_dat$time_point <- gsub("Gastrulation", "d) Early Gastrula", volume_dat$time_point)
volume_dat$time_point <- gsub("Planulae", "e) Planula", volume_dat$time_point)

volume_dat$treatment <- gsub("AMB", "pH 7.8", volume_dat$treatment) #replace AMB, l, and xl with pH 7.8, pH 7.6, and pH 7.3 for plotting
volume_dat$treatment <- gsub("xl", "pH 7.3", volume_dat$treatment) #replace AMB, l, and xl with pH 7.8, pH 7.6, and pH 7.3 for plotting
volume_dat$treatment <- gsub("l", "pH 7.6", volume_dat$treatment) #replace AMB, l, and xl with pH 7.8, pH 7.6, and pH 7.3 for plotting
volume_dat$treatment <- factor(volume_dat$treatment, levels=c("pH 7.8", "pH 7.6", "pH 7.3"))

df <- ggplot(volume_dat, aes(x = treatment, y = volume, color = treatment)) + #Square the sqrt volume to make size visually comparable across life stages
  ylab(expression("Volume " (mm^3))) + xlab("Treatment") + #Label axes
  ylim(0,0.1) +
  geom_jitter(alpha = 0.5) +
  geom_boxplot(alpha = 0) +
  facet_wrap(facets = vars(time_point), nrow = 2, ncol = 2, scales = "fixed") +
  theme_bw() + #Set background color
  theme(panel.border = element_blank(), # Set border
                     panel.grid.major = element_blank(), #Set major gridlines
                     panel.grid.minor = element_blank(), #Set minor gridlines
                     axis.text=element_text(size=12), #Set Y tick label size
                     axis.title=element_text(size=14), #Set Y tick label size
                     axis.line = element_line(colour = "black"), #Set axes color
        strip.text = element_text(size = 14, face = 'bold'), #set facet size
        legend.position = "none",
        plot.background=element_blank()) + #Set the plot background
  scale_color_manual(values=c("pH 7.8" ="cadetblue", "pH 7.6"="palevioletred", "pH 7.3"="indianred3")); df #set treatment colors
```
Add annotations to plot
```{r}
ann_text.a <- data.frame(
  treatment = c("pH 7.8", "pH 7.6", "pH 7.3", "pH 7.8", "pH 7.8"),
  volume = c(0.075,0.075,0.075,0.09,0.09),
  lab = c("a", "a", "a", "a", "a"),
  time_point = factor(c("c) Fertilized Egg", "c) Fertilized Egg", "c) Fertilized Egg", "d) Early Gastrula", "e) Planula"),levels = c("b) Unfertilized Egg","c) Fertilized Egg","d) Early Gastrula", "e) Planula")))
ann_text.b <- data.frame(
  treatment = c("pH 7.6", "pH 7.3", "pH 7.6", "pH 7.3"),
  volume = c(0.09,0.09,0.09,0.09),
  lab = c("b", "b", "b", "b"),
  time_point = factor(c("d) Early Gastrula", "d) Early Gastrula", "e) Planula", "e) Planula"),levels = c("b) Unfertilized Egg","c) Fertilized Egg","d) Early Gastrula", "e) Planula")))
df <- df + geom_text(data = ann_text.a,label = "a", color = "black", hjust = 5) + geom_text(data = ann_text.b,label = "b", color = "black", hjust   = 5) 
```


Get summary stats for methods section.
```{r}
summarySE(volume_dat, measurevar = "volume", groupvars = c("time_point", "treatment"))
```


```{r}
fig_2 <- grid.arrange(final_cell_cleavage, df, ncol = 2, nrow = 1, widths = c(5,12), clip = "off")
ggsave("0-Developmental_progression_and_size/Output/fig2_embryo_development.pdf", fig_2, width = 16, height = 8, units = c("in"))
```