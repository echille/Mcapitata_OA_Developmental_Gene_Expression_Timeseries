...
See what modules planula DEGs belong to
```{r}
geneInfo <- read_csv("3-WGCNA/Output/geneInfo.csv")[,-1]
geneInfo <- subset(geneInfo, select=c("gene_id", "moduleColor"))

clust1 <- filter(cluster_res, cluster=="1")
clust2 <- filter(cluster_res, cluster=="2")

geneInfo.C1 <- filter(geneInfo, gene_id%in%clust1$gene_id)
geneInfo.C1$yn <- c(1)
geneInfo.C1 <- pivot_wider(geneInfo.C1, names_from = "moduleColor", id_cols = "gene_id", values_from = "yn")
geneInfo.C1[is.na(geneInfo.C1)] <- c(0)
rownames(geneInfo.C1) <- geneInfo.C1$gene_id
geneInfo.C1 <- geneInfo.C1[,-c(1)]
colSums(geneInfo.C1)

geneInfo.C2 <- filter(geneInfo, gene_id%in%clust2$gene_id)
geneInfo.C2$yn <- c(1)
geneInfo.C2 <- pivot_wider(geneInfo.C2, names_from = "moduleColor", id_cols = "gene_id", values_from = "yn")
geneInfo.C2[is.na(geneInfo.C2)] <- c(0)
rownames(geneInfo.C2) <- geneInfo.C2$gene_id
geneInfo.C2 <- geneInfo.C2[,-c(1)]
colSums(geneInfo.C2)
```
...
# 
# ann_colors <- list(treatment= c(amb="slateblue", l="indianred3", xl="deeppink4"), time_point=c(Fertilized_egg="darkgreen", Cleavage="cadetblue4", Prawn_chip="deepskyblue", Early_gastrula="cornflowerblue", Planula="darkorchid4")) #color-blind friendly
# 
