...
#gene count matrix
gcount <- as.data.frame(read.csv("1-QC-Align-Assemble/Output/gene_count_matrix.csv", row.names="gene_id"))
...
ggplot(gPCAdata_fe, aes(PC1, PC2, color=treatment)) + 
  ...
  scale_color_manual(values = c(amb="cadetblue3", l="palevioletred", xl="indianred3")) +
  coord_fixed() +
...
##### Pre-filter gene counts
Pre-filtering our dataset to reduce the memory size dataframe, increase the speed of the transformation and testing functions, and improve quality of statistical analysis by removing low-coverage counts. Removed counts could represent outliers in the data and removing these improves sensitivity of statistical tests. Here we will filter out the samples from time points egg, morula, and gastrula because we only have samples from the ambient treatments of these three life stages. We will also apply a filter so only transcripts with 
```{r}
...
write.csv(pln_DEGs.l_vs_amb, "2-Differential-Gene-Expression-Analysis/Output/pln_DEGs_l_vs_amb.csv")
...
write.csv(pln_DEGs.xl_vs_amb, "2-Differential-Gene-Expression-Analysis/Output/pln_DEGs_xl_vs_amb.csv")
...
write.csv(pln_DEGs.xl_vs_l, "2-Differential-Gene-Expression-Analysis/Output/pln_DEGs_xl_vs_l.csv")
...
```{r}
ann_colors <- list(treatment= c(amb="cadetblue3", l="palevioletred", xl="indianred3")) #color-blind friendly
...
pln_DEGheatmap <- pheatmap(matpln, scale= "row", annotation_col=df_pln, border_color = "black",
              ...
              filename = "2-Differential-Gene-Expression-Analysis/Output/OA-PLN_DEGheatmap.png") 
pln_DEGheatmap
```
...
write.csv(pln_DEGs_all, file="2-Differential-Gene-Expression-Analysis/Output/pln_DEGs_all.csv")
```
...
ann_colors <- c("cadetblue3", "palevioletred","indianred3") #color-blind friendly)
...
venn.diagram(
  ...
  category.names = c("Low vs Ambient" , "Extreme Low vs Ambient" , "Extreme Low vs Low"),
        filename = '2-Differential-Gene-Expression-Analysis/Output/OA_pln_DEGs.png',
  output=TRUE,
...
### Plot a heatmap of the planula DEGs with all other timepoints. **May exclude this because developmental gene expression patterns mask treatment patterns in planula in the heatmap.**

#### First, get vst-transformed expression for all life stages in the planula DEGs

Remove lowly-expressed genes using the PoverA filter applied above
```{r}
# #create filter for the counts data
...
# 
# DEGlist <- unique(pln_DEGs_all$gene_id)
# gcount_filt$gene_id <- rownames(gcount_filt)
# gcount_filt <- filter(gcount_filt, gene_id%in%DEGlist)
# gcount_filt <- subset(gcount_filt, select= -c(gene_id, X119, X120, X121))
# head(gcount_filt)
```
Create a DESeqDataSet design from gene count matrix and labels. We will use this to make an attributes for each sample to make plotting easier.
```{r}
# treatmentinfo$treatment <- as.factor(treatmentinfo$treatment)
# treatmentinfo$time_point <- as.factor(treatmentinfo$time_point)
# 
# #Set DESeq2 design
...
We know from before that our size factors are all less than 4, so we can use VST!
```{r}
# DEGvst <- vst(gdds, blind=FALSE, nsub = nrow(counts(gdds))) #apply a variance stabilizing transforamtion to minimize efplncts of small counts and normalize wrt library size
# dim(assay(DEGvst))
```
...
# 
# ann_colors <- list(treatment= c(amb="cadetblue3", l="palevioletred", xl="indianred3"), time_point=c(Fertilized_egg="darkgreen", Cleavage="cadetblue4", Prawn_chip="deepskyblue", Early_gastrula="cornflowerblue", Planula="darkorchid4")) #color-blind friendly
# 
# df <- as.data.frame(subset(treatmentinfo, select = c(sample_id, time_point, treatment)))
# rownames(df) <- df$sample_id
# df <- subset(df, select= -c(sample_id))
# df <- filter(df, time_point != "Unfertilized_egg")
# 
# pheatmap(mat, scale= "row", annotation_col=df, border_color = "black",
#                            annotation_colors = ann_colors,
#                            clustering_distance_rows="euclidean", clustering_method = "average", 
#                            show_rownames =FALSE,
#                            show_colnames =FALSE,
#                            cluster_cols = FALSE,
#                            cluster_rows = TRUE, cutree_cols = 3, cutree_rows = 2) 
```
